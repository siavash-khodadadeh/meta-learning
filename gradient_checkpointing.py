...
def gradients(ys, xs, grad_ys=None, checkpoints='collection', **kwargs):
    ...
    if type(checkpoints) is not list:
        if checkpoints == 'collection':
            ...
            checkpoints = ge.filter_ts(fwd_ops, True)  # get the tensors
            checkpoints = [t for t in checkpoints if '/read' not in t.name]
            checkpoints = set(checkpoints) - set(xs) - set(ys)
